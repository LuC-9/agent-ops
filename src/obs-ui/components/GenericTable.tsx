...
export default function GenericTable({
  rows, loading, onRowClick, exportName = "export", columns: customColumns, expandable, scrollY,
  total, page, pageSize, onPageChange, toolbarLeft, controlsAlign = "left",
}: {
  rows: any[]; loading?: boolean; onRowClick?: (r: any) => void; exportName?: string;
  columns?: any[]; expandable?: any; scrollY?: number;
  ...
  const columns = customColumns ?? keys.map((k) => ({
    title: titleize(k),
    dataIndex: k,
    key: k,
    ellipsis: true,
    ...
    <>
      <div
        style={{
          marginBottom: 8,
          display: "flex",
          justifyContent: controlsAlign === "right" ? "space-between" : "flex-start",
          alignItems: "center",
          gap: 8,
        }}
      >
        {controlsAlign === "right" ? <div>{toolbarLeft}</div> : null}
        <Space size={8}>
          <Tooltip title="Download CSV">
            <Button size="small" type="text" icon={<DownloadOutlined />} onClick={download} disabled={!rows.length} />
          </Tooltip>
          <span style={{ color: "#999", fontSize: 12 }}>{rows.length} rows</span>
        </Space>
      </div>
      <Table
        size="small"
        ...
        pagination={
          server
            ...
            : scrollY ? false : { pageSize: 25, showSizeChanger: true }
        }
