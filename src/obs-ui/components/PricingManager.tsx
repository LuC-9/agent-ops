...
export default function PricingManager({ open, onClose }: { open: boolean; onClose: () => void }) {
        ...
        try {
          ...
          if (e?.response?.status === 409) {
            modal.confirm({
              ...
              content: `Replace the current active price for "${name}"?`,
              okText: "Replace", okButtonProps: { danger: true }, cancelText: "Cancel",
              onOk: async () => {
                try { await doSave({ ...v, model_prefix: name }, true); message.success("Price updated"); form.resetFields(); form.setFieldsValue({ active: true }); load(); }
  ...
  const label = (prefix: string) =>
    !prefix || prefix === "default" ? "Default (unmatched models)" : prefix;

  const columns = [
    { title: "Model", dataIndex: "model_prefix", render: (v: string) => label(v) },
  ...
  return (
    <Modal open={open} onCancel={onClose} footer={null} width={820} title="LLM pricing" destroyOnHidden>
      <Typography.Paragraph type="secondary">
        Cost uses the longest matching prefix (e.g. <b>gemini</b> covers gemini-2.5-flash). Unmatched models use Default.
      ...
      <Typography.Title level={5} style={{ marginTop: 12 }}>Add a model</Typography.Title>
      <Form form={form} preserve={false} layout="inline" initialValues={{ active: true }} onFinish={onSave} style={{ rowGap: 8, flexWrap: "wrap" }}>
        <Form.Item name="model_prefix" rules={[{ required: true, message: "name" }]}>
          <Input placeholder="e.g. gemini-2.5-pro" style={{ width: 240 }} />
        </Form.Item>
