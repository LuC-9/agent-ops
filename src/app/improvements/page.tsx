...
export default function ImprovementsPage() {
  ...
  async function load() {
    const [imp, ag] = await Promise.all([
      fetch("/api/improvements").then((r) => r.json()),
      fetch("/api/agents").then((r) => r.json()),
    ]);
    setItems(imp.improvements ?? []);
    setAgents(ag.agents ?? []);
  }

  useEffect(() => {
    load().catch(() => setNote("Failed to load improvements"));
  }, []);
    ...
    <div className="mx-auto max-w-4xl space-y-6 p-4 md:p-8">
      <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            The observability agent reads error logs, low-accuracy traces, and system prompts, then proposes graph and prompt patches.
          </p>
        </div>
      ...
      <div className="space-y-3">
        {items.map((item) => {
          ...
          return (
            <article key={item.id} className="rounded-lg border border-white/10 bg-[#10202c] p-4">
